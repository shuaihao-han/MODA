...
def _min_area_polygons_cpu(pts):
    """CPU fallback for min_area_polygons to avoid CUDA crashes."""
    pts_cpu = pts.detach().float().cpu().numpy()
    polys = np.zeros((pts_cpu.shape[0], 8), dtype=np.float32)
    for i in range(pts_cpu.shape[0]):
        p = pts_cpu[i].reshape(-1, 2).astype(np.float32)
        rect = cv2.minAreaRect(p)
        box = cv2.boxPoints(rect)
        polys[i] = box.reshape(-1)
    return torch.from_numpy(polys).to(pts.device)
...
class OrientedRepPointsHead(BaseDenseHead):
    ...
    def forward_single(self, x):
        ...
        Returns:
            cls_out (torch.tensor): classification score prediction (分类分数预测)
            pts_out_init (torch.tensor): initial point sets prediction (初始点集预测)
            pts_out_refine (torch.tensor): refined point sets prediction (refined 点集预测)
            base_feat: single-level feature as the basic feature map (单层特征作为基本特征图, 就是原始的特征图)
        """
        ...
        for cls_conv in self.cls_convs:
            cls_feat = cls_conv(cls_feat)   # 形状与原始特征图相同
        ...
        cls_out = self.reppoints_cls_out(
            self.relu(self.reppoints_cls_conv(cls_feat, dcn_offset)))   # [B, class_num, H, W] 特征图上每个点对应的类别分数(8 个类别的置信度)
    ...
    def _get_bboxes_single(self,
        ...
        for level_idx, (cls_score, points_pred, points) in enumerate(
            ...
            pts = pts + torch.rand(pts.shape).to(pts.device) * 0.00001
            if os.getenv("SAFE_MINAREA", "0") == "1":
                finite = torch.isfinite(pts)
                if not finite.all():
                    pts = torch.where(finite, pts, torch.zeros_like(pts))
                pts = torch.clamp(pts, min=-1e4, max=1e4)
                pts = pts.contiguous()
                polys = _min_area_polygons_cpu(pts)
            else:
