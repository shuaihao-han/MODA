...
class FixedRotate(object):
    """Deterministically rotate the image by a fixed angle (degrees).

    This is intended for test-time robustness evaluation where we want to
    rotate the input images but keep the rest of the pipeline unchanged.
    It does NOT require annotations in `results`.

    Notes:
    - The implementation mirrors the image rotation behavior in PolyRandomRotate
      (offset = -0.5) when `auto_bound=False`.
    - Output image keeps the same H/W (no auto-bound).
    """

    def __init__(self, angle=0, border_value=0, interpolation='bilinear'):
    ...
    def __call__(self, results):
        ...
        h, w = img.shape[:2]
        if h == 0 or w == 0 or self.angle % 360 == 0:
            # Still mark the angle for downstream debugging.
            results['rotate'] = False
            results['rotate_angle'] = self.angle
            return results

        # Match PolyRandomRotate.apply_image behavior (rotate about center-0.5).
        image_center = np.array((w / 2, h / 2), dtype=np.float32)
        rm = cv2.getRotationMatrix2D(tuple(image_center - 0.5), self.angle, 1.0)

        flags = cv2.INTER_NEAREST if self.interpolation == 'nearest' else cv2.INTER_LINEAR
        img = np.ascontiguousarray(img)
        rotated = cv2.warpAffine(
            img, rm, (w, h), flags=flags, borderValue=self.border_value)

        results['img'] = rotated
        # Keep shapes consistent for downstream transforms.
        if rotated.ndim == 2:
            results['img_shape'] = rotated.shape
            results['ori_shape'] = rotated.shape
        else:
            results['img_shape'] = rotated.shape
            results['ori_shape'] = rotated.shape
        results['rotate'] = True
    ...
    def __repr__(self):
        return f'{self.__class__.__name__}(angle={self.angle}, interpolation={self.interpolation})'
