...
model = dict(
    ...
    backbone=dict(
        ...
        num_stages=4,
        out_indices=(0, 1, 2, 3),
        # frozen_stages=1,   # [origin]: frozen_stages=1 冻结backbone的第一个stage的参数
        zero_init_residual=False,
        norm_cfg=dict(type='BN', requires_grad=True),
        norm_eval=True,
        style='pytorch',
        init_cfg=dict(type='Pretrained', checkpoint='work_dirs/mmrotate_stuff/pretrained_weight/resnet50_band_table_msod_ordered3.pth')),   # [origin]: checkpoint='torchvision://resnet50'
    neck=dict(
        type='FPN',
...
train_pipeline = [
    ...
    dict(type='RResize', img_scale=(1200, 1200)),  # [origin]: dict(type='RResize', img_scale=(1024, 1024)),
    dict(
        type='RRandomFlip',
...
# 将除backbone部分的网络参数的学习率设置为backbone的1/10,以使网络专注于backbone参数的更新以适应我们的数据集
# 同时不至于是backbone部分的参数过于适应单一检测网络，失去泛化性能
optimizer = dict(lr=0.001, 
                 paramwise_cfg=dict(custom_keys={'neck': dict(lr_mult=0.1, decay_mult=1.0), 
                                                 'bbox_head': dict(lr_mult=0.1, decay_mult=1.0)}))     # [origin]: lr=0.008
evaluation = dict(interval=4, metric='mAP')   # [origin]: interval=1 
