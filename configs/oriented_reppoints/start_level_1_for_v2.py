...
model = dict(
    ...
    neck=dict(
        ...
        out_channels=256,
        start_level=1,   # 从第几个输入特征图开始构建 FPN 金字塔, start_level=1 意味着从输入的第二个特征图（即 C3, 通道数为 512）开始构建金字塔
...
optimizer = dict(type='AdamW', lr=0.00035, betas=(0.9, 0.999), weight_decay=0.01)   # 使用AdamW优化器
...
# fp16 settings
fp16 = dict(loss_scale='dynamic')   # 设置混合精度训练(降低显存占用)，loss_scale='dynamic'表示动态调整loss scale
# 防止未产生损失的参数引发问题
